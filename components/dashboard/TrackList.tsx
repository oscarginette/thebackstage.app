      ...
      <div className="bg-card border border-border rounded-3xl p-8">
        {!hasSoundCloudId ? (
          <div className="text-center py-8">
            <h3 className="text-lg font-semibold text-foreground/60 mb-2">Conecta tu SoundCloud</h3>
          ...
          <div className="text-center py-8">
            ...
            <h3 className="text-lg font-semibold text-foreground/60 mb-2">Explora tu música</h3>
          ...
          <>
            <div className="flex items-center justify-between mb-6">
              <div>
                <h3 className="text-xl font-serif text-foreground">Feed de Tracks</h3>
              ...
              <button
                onClick={onLoadAll}
                disabled={loading}
                className="px-4 py-2 rounded-full text-xs font-medium border border-border text-foreground hover:bg-muted disabled:opacity-50 transition-colors"
              >
          ...
          <div className="grid grid-cols-1 gap-2 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
            {tracks.length === 0 ? (
              ...
              tracks.map((track) => (
                <div
                  key={track.trackId}
                  className="group relative flex items-center gap-3 p-3 bg-secondary rounded-2xl border border-transparent hover:border-border hover:bg-card transition-all duration-300"
                >
                  {/* Artwork */}
                  <div className="relative shrink-0 w-12 h-12 rounded-lg overflow-hidden shadow-sm">
                     {track.coverImage ? (
                      ...
                      <div className="w-full h-full bg-muted flex items-center justify-center">
                         <svg className="w-6 h-6 text-muted-foreground" fill="currentColor" viewBox="0 0 24 24"><path d="M12 3v10.55c-.59-.34-1.27-.55-2-.55-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4V7h4V3h-6z" /></svg>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <h3 className="font-serif text-base text-foreground leading-tight truncate">
                        {track.title}
                      </h3>
                      {track.alreadySent && (
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <a href={track.url} target="_blank" className="text-[11px] font-medium text-accent hover:underline">
                        Ver en SC ↗
                      </a>
                    </div>
                  </div>
                  ...
                  <div className="shrink-0">
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-foreground text-background text-[11px] font-bold rounded-full hover:bg-foreground/90 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 shadow-md shadow-foreground/10 hover:-translate-y-0.5"
                    >
